check for git config --global push.default simple
...
_SADM_BASH_CMDS_FILE = '3po-bash-commands.sh'
...
def _setup_windows_path():
    ...
    def check_system_path():
        ...
        for p in path:
            if os.path.isfile(os.path.join(p, '3po.py')):
    ...
    3popath = ioutil.os_norm_seps(APP_FOLDER).lower().rstrip('\\')
    3po_in_path = False
    path = get_user_path()
    bad3po = []
    for p in path:
        if p.lower() == 3popath:
            3po_in_path = True
        elif os.path.isfile(os.path.join(p, '3po.py')):
            eprintc('\nA different copy of 3po was found in your path and will be removed: %s' % p, WARNING_COLOR)
            bad3po.append(p)
    ...
    if os.name == 'nt':
        ...
        if os.path.isfile(bash_profile):
            ...
            if txt.find(_SADM_BASH_CMDS_FILE) > -1:
                ...
                return
        print('')
        if 3po_prompt.prompt_bool('Add convenience functions like "sb cr" and "sb build" to %s' % bash_profile, 'y'):
            print('')
            cmds_path, updated = _update_bash_funcs()
            txt = read_file(bash_profile).rstrip()
            txt += '\n\n# Convenience functions installed by 3po\ntest -r %s && source %s\n\n' % (cmds_path, cmds_path)
            save(bash_profile, txt)
            eprintc('Restart bash to get updates.\n', WARNING_COLOR)

def _update_bzr_plugin():
    plugindir = os.path.join(vcs.PRIVATE_VCS_FOLDER, 'plugins', 'perfectsearch')
    ...
    if not os.path.isdir(BUILDSCRIPTS_FOLDER):
        ...
        # Normally, we'll be installing from a built 3po and a built bzr-plugins folder,
...
def _check_required_tools():
    print('\nChecking tools required by 3po...\n')
    rtools = []
...
owner of the svn project cooperate to fix it. Many 3po settings are hard-coded
for such machines.
''')
        config.is_canonical_machine = 3po_prompt.prompt_bool('Treat this box as a canonical build machine?', _get_prompt_bool_repr(config.is_canonical_machine))
        if config.is_canonical_machine:
            config.auto_add_sandboxes = 3po_prompt.prompt_bool('Automatically add sandboxes?', _get_prompt_bool_repr(config.auto_add_sandboxes))
            config.branches_to_auto_add = 3po_prompt.prompt('Which branches would you like to add sandboxes from.(Give a list seperated by commas e.i. trunk,push_feature,... or all for all branches.)', config.branches_to_auto_add)
        else:
...
def _check_remoting_info():
    if 3po_prompt.prompter.get_mode() == 3po_prompt.AUTOCONFIRM_MODE:
        return
...
''')
    config.username_for_remote_access = 3po_prompt.prompt('Username for remote login', config.username_for_remote_access)
    config.ipaddr = 3po_prompt.prompt('IP address', config.ipaddr)
    config.ssh_port = 3po_prompt.prompt('Port for ssh', config.ssh_port)
    config.remote_desktop_port = 3po_prompt.prompt('Port for remote desktop', config.remote_desktop_port)
    config.remoting_instructions = 3po_prompt.prompt('''
...
def _check_loc(config, prop_name, msg, validator=None, default_func=None):
    ...
    while (not is_location):
        location = 3po_prompt.prompt(msg, default_val)
...
''')
        config.auto_update_dev_sandboxes = 3po_prompt.prompt_bool('Auto-get code in *dev before work each morning?', _get_prompt_bool_repr(config.auto_update_dev_sandboxes))
...
''')
    rng = 3po_prompt.prompt('Hours for running continuous sandboxes (e.g., 0100 to 2200; "*" = all)', defValue)
    if rng and (rng == '*' or rng == 'all'):
        rng = ''
    config.schedule_continuous_manually = 3po_prompt.prompt_bool('\nSchedule continuous sandboxes manually?', _get_prompt_bool_repr(config.schedule_continuous_manually))
...
in a JDBC database. Normally, the db is on db1.perfectsearchcorp.com, the
user is named "perftest-writer", and the password is stored in passpack...
''')
    defValue = config.perf_log_db_url
    if defValue == STD_PERFTEST_DBURL:
        defValue = 'std'
    x = 3po_prompt.prompt('URL for perf test db ("std", custom value, or "none" to disable)', defValue)
    if x:
        if x == 'std':
    ...
    if x:
        x = 3po_prompt.prompt('Password for perf test db', config.perf_log_password)
...
def _check_role():
    print('''
Some 3po features are useful only on dedicated build machines. Others are
...
''')
    while True:
        machine_role = 3po_prompt.prompt('Machine role?', config.machine_role)
        if machine_role:
...
def _check_writable_app_folder():
    ...
    try:
        with open(tmp, 'w') as f:
            f.write('this is just a test to see if 3po has write access to its own folder')
            f.close()
            time.sleep(0.1)
            os.remove(tmp)
...
def _check_app_checked_out():
    if not os.path.isdir(os.path.join(APP_FOLDER, '.bzr')):
        eprintc('''
Sadm has not been installed with a bzr checkout command. This will prevent 3po
from updating itself. Recommended install command for 3po is:
    bzr co --lightweight %s/3po/built.%s/trunk <installdir>''' % (
    ...
    if os.name == 'nt':
        if config.uac_disabled is None:
            if user_has_admin_privileges():
                config.uac_disabled = 3po_prompt.prompt_bool(
